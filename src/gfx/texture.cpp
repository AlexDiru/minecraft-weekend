...
void texture_load_pixels(char *path, u8 **pixels_out, size_t *width_out, size_t *height_out) {
    ...
    *pixels_out = (u8*)malloc(width * height * 4);
...
Texture texture_create_from_pixels(u8 *pixels, size_t width, size_t height) {
    Texture self = {
...
Atlas atlas_create_from_texture(Texture texture, ivec2s sprite_size) {
    Atlas self;
...
Atlas atlas_create(char *path, ivec2s sprite_size) {
    Atlas self;
...
void atlas_get(Atlas self, ivec2s pos, vec2s *uv_min, vec2s *uv_max) {
    vec2s p_min = (vec2s) {{

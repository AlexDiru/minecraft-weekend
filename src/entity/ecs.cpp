...
#define ECS_TAG(_p) *((ECSTag *) (((int *) (_p)) - ECS_TAG_SIZE))
#define ECS_PTAG(_p) ((ECSTag *) (((int *) (_p)) - ECS_TAG_SIZE))
...
#define ECSCL_GET(_plist, _i) ({\
        ...
        static_cast<char*>((_pl)->components) + ((_i) * ECSCL_ELEMENT_SIZE(_pl)) + ECS_TAG_SIZE;\
    })
...
// create a new entity
struct Entity ecs_new(ECS *self) {
    // entity index
    size_t i;
    ...
    if (i == self->capacity) {
        ...
        for (size_t j = 0; j <= ECSCOMPONENT_LAST; j++) {
            ...
            list->components = (void*)realloc(
                list->components,
                self->capacity * ECSCL_ELEMENT_SIZE(list));

            // initialize new allocation
            memset(
                static_cast<char*>(list->components) + (old_capacity * ECSCL_ELEMENT_SIZE(list)), 0,
...
void ecs_init(ECS *self, World *world) {
    self->capacity = 64;
    self->ids = (EntityId*)calloc(self->capacity, sizeof(EntityId));

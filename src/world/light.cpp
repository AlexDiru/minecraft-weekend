...
static void add_propagate(
    World *world, LightQueue *queue,
    u32 mask, u32 offset, enum PropagationType type) {
    while (queue->size != 0) {
        LightNode node = DEQUEUE(queue);
        ...
        for (int d = DOWN; d >= 0; d--) {
            ...
            Block n_block = BLOCKS[chunk_data_to_block(n_data)];
            ...
            if ((n_val != 0 || n_block.transparent) &&
                ...
                ENQUEUE(queue, ((LightNode) { .pos = n_pos }));
            }
        }
    }
}
...
static void remove_propagate(
    World *world, LightQueue *queue, LightQueue *prop_queue,
    u32 mask, u32 offset, enum PropagationType type) {
    while (queue->size != 0) {
        LightNode node = DEQUEUE(queue);
        u32 value = node.value;

        // 6 directions, C version iterated enum
        for (int d = 0; d < 6; d++) {
            ivec3s n_pos = glms_ivec3_add(node.pos, directionIndexToIVec3s(d));
            ...
            if ((n_light & mask) != 0 &&
                ...
                ENQUEUE(queue, ((LightNode) { .pos = n_pos, .value = n_value }));
            } else if (n_value >= value) {
                ENQUEUE(prop_queue, ((LightNode) { .pos = n_pos }));
            }
        }
    }
}
...
static void remove_channel(
    ...
    ENQUEUE(&queue, ((LightNode) { .pos = pos, .value = (light & mask) >> offset }));
...
void light_update(World *world, ivec3s pos) {
    ...
    for (size_t i = 0; i < 5; i++) {
        ...
        for (int d = 0; d < 6; d++) {
            ivec3s pos_n = glms_ivec3_add(pos, directionIndexToIVec3s(d));

            if (!BLOCKS[world_get_block(world, pos_n)].transparent) {
                continue;
            }

            ENQUEUE(&queue, ((LightNode) { .pos = pos_n }));
        }
...
void light_apply(Chunk *chunk) {
    ...
    LightQueue sunlight_queue = { .size = 0 },
    ...
    for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
        for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
            ...
            for (s64 y = CHUNK_SIZE.y - 1; y >= 0; y--) {
                ivec3s pos_c = (ivec3s) {{ (int)x, (int)y, (int)z }},
    ...
    for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
        for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
            pos = glms_ivec3_add(chunk->position, (ivec3s) {{ (int)x, -1, (int)z }});
            light = world_get_light(chunk->world, pos);

            if (light != 0) {
                ENQUEUE(&torchlight_queue, ((LightNode) { .pos = pos, .value = light }));
            }

            pos = glms_ivec3_add(chunk->position, (ivec3s) {{ (int)x, (int)CHUNK_SIZE.y, (int)z }});
            light = world_get_light(chunk->world, pos);
    ...
    for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
        for (s64 y = 0; y < CHUNK_SIZE.y; y++) {
            pos = glms_ivec3_add(chunk->position, (ivec3s) {{ (int)x, (int)y, -1 }});
            light = world_get_light(chunk->world, pos);
            ...
            pos = glms_ivec3_add(chunk->position, (ivec3s) {{ (int)x, (int)y, (int)CHUNK_SIZE.z }});
            light = world_get_light(chunk->world, pos);
    ...
    for (s64 y = 0; y < CHUNK_SIZE.y; y++) {
        for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
            pos = glms_ivec3_add(chunk->position, (ivec3s) {{ -1, (int)y, (int)z }});
            light = world_get_light(chunk->world, pos);
            ...
            ivec3s vec;
            vec.x = (int)CHUNK_SIZE.x;
            vec.y = (int)y;
            vec.z = (int)z;
            pos = glms_ivec3_add(chunk->position, vec);
            light = world_get_light(chunk->world, pos);
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        for (size_t j = 0; j < torchlight_queue.size; j++) {
            LightNode n = torchlight_queue.elements[i];
            if ((n.value & mask) != 0) {
                ENQUEUE(&queue, ((LightNode) { .pos = n.pos }));
            }
        }

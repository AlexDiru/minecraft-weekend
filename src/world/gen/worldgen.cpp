...
BiomeData BIOME_DATA[BIOME_LAST + 1] = {
    [OCEAN] = {
        .top_block = SAND,
        .bottom_block = SAND,
        .roughness = 1.0f,
...
static u32 _get(Chunk *chunk, s32 x, s32 y, s32 z) {
    ivec3s p = (ivec3s) {{ x, y, z }};
    if (chunk_in_bounds(p)) {
        return (u32)chunk_get_block(chunk, p);
    } else {
        return (u32)world_get_block(chunk->world, glms_ivec3_add(chunk->position, p));
    }
}

static void _set(Chunk *chunk, s32 x, s32 y, s32 z, u32 d) {
    BlockId blockId = static_cast<BlockId>(d);
    ivec3s p = (ivec3s){{x, y, z}};
    if (chunk_in_bounds(p)) {
        chunk_set_block(chunk, p, blockId);
    }
    
    ivec3s p_w = glms_ivec3_add(chunk->position, p);
    if (!world_contains(chunk->world, p_w)) {
        world_append_unloaded_block(chunk->world, p_w, blockId);
    } else {
        world_set_block(chunk->world, p_w, blockId);
    }
}
    ...
    if (!heightmap->flags.generated) {
        ...
        for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
            for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
                ...
                heightmap->worldgen_data[x * CHUNK_SIZE.x + z] = (WorldgenData) {
...
#define WG_GET_H(_x, _z)\
    ...
    for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
        for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
            WorldgenData data = heightmap->worldgen_data[x * CHUNK_SIZE.x + z];
            s64 h = data.h;
            enum Biome biome = static_cast<Biome>(data.b);
            BiomeData biome_data = BIOME_DATA[biome];
            ...
            for (s64 y = 0; y < CHUNK_SIZE.y; y++) {
                ...
                chunk_set_block(chunk, (ivec3s) {{ (int)x, (int)y, (int)z }}, block);

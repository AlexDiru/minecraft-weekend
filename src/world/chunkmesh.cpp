...
static void buffer_init(ChunkMeshBuffer *self, enum BufferType type) {
    memset(self, 0, sizeof(struct ChunkMeshBuffer));
    self->type = type;
    self->capacity = BUFFER_SIZES[type];
}

static void buffer_destroy(ChunkMeshBuffer *self) {
    if (self->data != NULL) {
        free(self->data);
    }
}
...
struct ChunkMesh *chunkmesh_create(Chunk *chunk) {
    struct ChunkMesh *self = (ChunkMesh*)calloc(1, sizeof(struct ChunkMesh));
    self->chunk = chunk;
    self->vao = vao_create();
    ...
    for (size_t i = 0; i <= BUFFER_TYPE_LAST; i++) {
        buffer_init(&self->buffers[i], static_cast<BufferType>(i));
    }
...
static void buffer_prepare(ChunkMeshBuffer *self) {
    if (self->data == NULL) {
...
void chunkmesh_prepare(ChunkMesh *self) {
    self->vertex_count = 0;
...
static void chunkmesh_flip_buffers(ChunkMesh *self) {
...
static void chunkmesh_finalize_data(ChunkMesh *self) {
...
static void chunkmesh_finalize_indices(ChunkMesh *self) {
...
static int depth_cmp(const Face *a, const Face *b) {
        ...
        if(block_id != AIR) {
            ...
            switch (params.block->mesh_type) {
                case BLOCKMESH_SPRITE:
                    atlas_get(
                        block_atlas->atlas,
                        params.block->get_texture_location(chunk->world, pos_w, static_cast<Direction>(0)),
                ...
                case BLOCKMESH_LIQUID:
                    ...
                    for (int d = 0; d < 6; d++) {
                        ...
                        ivec3s d_v = directionIndexToIVec3s(d);
                        ...
                        if (params.block->mesh_type == BLOCKMESH_CUSTOM ||
                            ...
                            atlas_get(
                                block_atlas->atlas,
                                params.block->get_texture_location(chunk->world, pos_w, direction),
                                &params.uv_min, &params.uv_max);
